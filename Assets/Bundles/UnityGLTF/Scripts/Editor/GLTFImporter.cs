    ...
    public override void OnImportAsset(AssetImportContext ctx) {
      ...
      try {
        ...
        if (_removeEmptyRootObjects) {
          ...
          while (gltfScene.transform.childCount == 1 && gltfScene.GetComponents<Component>().Length == 1) {
        ...
        if (_importMaterials) {
          ...
          var materials = renderers.SelectMany(
              r => {
                return r.sharedMaterials.Select(
                    mat => {
          ...
          var textures = materials.SelectMany(
              mat => {
