...
namespace UnityGLTF.Extensions {
  public static class SchemaExtensions {
    ...
    public static bool CoordinateSpaceConversionRequiresHandednessFlip {
      get {
        return CoordinateSpaceConversionScale.X * CoordinateSpaceConversionScale.Y * CoordinateSpaceConversionScale.Z
               < 0.0f;
      }
    }
    ...
    public static void GetUnityTRSProperties(
        this Node node,
        out Vector3 position,
        out Quaternion rotation,
        out Vector3 scale) {
      if (!node.UseTRS) {
        var unityMat = node.Matrix.ToUnityMatrix4x4Convert();
    ...
    /// <param name="rotation">unity rotation quaternion</param>
    /// <param name="scale">unity scale vector</param>
    public static void GetTRSProperties(
        this Matrix4x4 mat,
        out Vector3 position,
        out Quaternion rotation,
        out Vector3 scale) {
      ...
      var calculatedZ = Vector3.Cross(x, y);
      var mirrored = Vector3.Dot(calculatedZ, z) < 0.0f;
    ...
    /// <param name="scale">unity scale vector</param>
    public static void GetTRSProperties(
        this GLTF.Math.Matrix4x4 gltfMat,
        out Vector3 position,
        out Quaternion rotation,
        out Vector3 scale) {
    ...
    public static Quaternion ToUnityQuaternionConvert(this GLTF.Math.Quaternion gltfQuat) {
      ...
      var origAxis = new Vector3(gltfQuat.X, gltfQuat.Y, gltfQuat.Z);
      var axisFlipScale = CoordinateSpaceConversionRequiresHandednessFlip ? -1.0f : 1.0f;
      var newAxis = axisFlipScale * Vector3.Scale(origAxis, CoordinateSpaceConversionScale.ToUnityVector3Raw());
    ...
    public static GLTF.Math.Quaternion ToGltfQuaternionConvert(this Quaternion unityQuat) {
      ...
      var origAxis = new Vector3(unityQuat.x, unityQuat.y, unityQuat.z);
      var axisFlipScale = CoordinateSpaceConversionRequiresHandednessFlip ? -1.0f : 1.0f;
      var newAxis = axisFlipScale * Vector3.Scale(origAxis, CoordinateSpaceConversionScale.ToUnityVector3Raw());
    ...
    public static Matrix4x4 ToUnityMatrix4x4Convert(this GLTF.Math.Matrix4x4 gltfMat) {
      ...
      var unityMat = convert * rawUnityMat * convert;
      return unityMat;
    }
    ...
    public static Vector3 ToUnityVector3Convert(this GLTF.Math.Vector3 gltfVec3) {
      Vector3 coordinateSpaceConversionScale = CoordinateSpaceConversionScale.ToUnityVector3Raw();
      var unityVec3 = Vector3.Scale(gltfVec3.ToUnityVector3Raw(), coordinateSpaceConversionScale);
      return unityVec3;
    }
    ...
    public static GLTF.Math.Vector3 ToGltfVector3Raw(this Vector3 unityVec3) {
      var gltfVec3 = new GLTF.Math.Vector3(unityVec3.x, unityVec3.y, unityVec3.z);
      return gltfVec3;
    }

    public static GLTF.Math.Vector4 ToGltfVector4Raw(this Vector4 unityVec4) {
      var gltfVec4 = new GLTF.Math.Vector4(unityVec4.x, unityVec4.y, unityVec4.z, unityVec4.w);
      return gltfVec4;
    }
    ...
    public static GLTF.Math.Matrix4x4 ToGltfMatrix4x4Raw(this Matrix4x4 unityMat) {
      ...
      var rawGltfMat = new GLTF.Math.Matrix4x4(
          c0.X,
          c0.Y,
          c0.Z,
          c0.W,
    ...
    public static Vector2[] ToUnityVector2Raw(this GLTF.Math.Vector2[] inVecArr) {
      var outVecArr = new Vector2[inVecArr.Length];
      for (var i = 0; i < inVecArr.Length; ++i) {
    ...
    public static Vector3 ToUnityVector3Raw(this GLTF.Math.Vector3 vec3) { return new Vector3(vec3.X, vec3.Y, vec3.Z); }

    public static Vector3[] ToUnityVector3Raw(this GLTF.Math.Vector3[] inVecArr) {
      var outVecArr = new Vector3[inVecArr.Length];
      for (var i = 0; i < inVecArr.Length; ++i) {
    ...
    public static Vector4[] ToUnityVector4Raw(this GLTF.Math.Vector4[] inVecArr) {
      var outVecArr = new Vector4[inVecArr.Length];
      for (var i = 0; i < inVecArr.Length; ++i) {
    ...
    public static Color[] ToUnityColorRaw(this GLTF.Math.Color[] inColorArr) {
      var outColorArr = new Color[inColorArr.Length];
      for (var i = 0; i < inColorArr.Length; ++i) {
    ...
    public static int[] ToIntArrayRaw(this uint[] uintArr) {
      var intArr = new int[uintArr.Length];
      for (var i = 0; i < uintArr.Length; ++i) {
        var uintVal = uintArr[i];
    ...
    public static void ConvertVector3CoordinateSpace(
        ref AttributeAccessor attributeAccessor,
        GLTF.Math.Vector3 coordinateSpaceCoordinateScale) {
      for (var i = 0; i < attributeAccessor.AccessorContent.AsVertices.Length; i++) {
    ...
    public static Vector3[] ConvertVector3CoordinateSpaceAndCopy(
        Vector3[] array,
        GLTF.Math.Vector3 coordinateSpaceCoordinateScale) {
    ...
    /// <param name="attributeAccessor">The attribute accessor to modify</param>
    /// <param name="coordinateSpaceCoordinateScale">The coordinate space to move into</param>
    public static void ConvertVector4CoordinateSpace(
        ref AttributeAccessor attributeAccessor,
        GLTF.Math.Vector4 coordinateSpaceCoordinateScale) {
      for (var i = 0; i < attributeAccessor.AccessorContent.AsVec4s.Length; i++) {
    ...
    /// <param name="coordinateSpaceCoordinateScale">The specified coordinate space</param>
    /// <returns>The copied and converted coordinate space</returns>
    public static Vector4[] ConvertVector4CoordinateSpaceAndCopy(
        Vector4[] array,
        GLTF.Math.Vector4 coordinateSpaceCoordinateScale) {
    ...
    public static void FlipFaces(ref AttributeAccessor attributeAccessor) {
      for (var i = 0; i < attributeAccessor.AccessorContent.AsTriangles.Length; i += 3) {
        var temp = attributeAccessor.AccessorContent.AsUInts[i];
    ...
    public static int[] FlipFacesAndCopy(int[] triangles) {
      var returnArr = new int[triangles.Length];
    ...
    public static Matrix4x4[] ToUnityMatrix4x4(this GLTF.Math.Matrix4x4[] inMatrixArr) {
      var outMatrixArr = new Matrix4x4[inMatrixArr.Length];
      for (var i = 0; i < inMatrixArr.Length; ++i) {
