    ...
    class OnnxModel:MonoBehaviour{
        ...
        static void UseApi(){
            ...
            using (var session = new InferenceSession(modelPath))
            {
                ...
                var inputData = LoadTensorFromFile("bench.in"); // this is the data for only one input tensor for this model
        ...
        static float[] LoadTensorFromFile(string filename){
            ...
            using (var inputFile = new StreamReader(filename))             // read data from file
            {
                ...
                var dataStr = inputFile.ReadLine()?.Split(new[] { ',', '[', ']' }, StringSplitOptions.RemoveEmptyEntries);
